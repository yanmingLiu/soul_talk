...
class FBU {
  ...
  static Future<Map<String, String>?> _getConfigFromRemote() async {
    try {
      ...
      _log('Facebook SDK: id=$facebookAppId, token=$facebookClientToken');

      // 验证配置是否有效
      if (facebookAppId.isEmpty || facebookClientToken.isEmpty) {
        _log('❌Facebook SDK AppId.isEmpty');
        return null;
      }
  ...
  static Future<void> initializeWithRemoteConfig() async {
    try {
      ...
      if (config != null) {
        ...
        _log('❌ initializeFacebookSDK');
        // 不抛出异常，只是记录日志，避免阻塞应用启动
        return;
      }
    } catch (e) {
      _log('❌ initializeFacebookSDK: $e');
      // 不重新抛出异常，避免阻塞应用启动
      return;
    }
  }
  ...
  static Future<bool> checkNativeInitializationStatus() async {
    try {
      ...
      _log('❌ checkNativeInitializationStatus: $e');
      return false;
    }
  }
  ...
  static Future<void> initializeFacebookSDK({
    ...
    try {
      ...
      _log('Facebook SDK init result: $result');
    } on PlatformException catch (e) {
      _log('❌Facebook SDK init Error: ${e.message}');
  ...
  static Future<void> retryInitializationIfNeeded() async {
    try {
      ...
      _log('retryInitializationIfNeeded Error: $e');
    }
  }
}
